...
contract OP_Resolver is Ownable {
    ...
    function setDisputeOracle(address oracle) external {
        require(signers[msg.sender], "Not a signer");
        DisputeOracle = oracle;
    }
    ...
    function getValueFromStateProof(bytes32 stateRoot, address target, bytes32 slotPosition, bytes memory proofsBlob) public pure returns(bytes32) {
    ...
    function getBlockandRoot() public view returns(uint256, bytes32){
        uint256 index = (Dispute(DisputeOracle).gameCount()) - 1;
        uint32 gameType;
        uint64 timestamp;
        address gameAddr;
        (gameType, timestamp, gameAddr) = Dispute(DisputeOracle).gameAtIndex(index);
        uint256 blockNumber = Game(gameAddr).l2BlockNumber();
        bytes32 outputRoot = Game(gameAddr).rootClaim();
        return (blockNumber, outputRoot);
    ...
    function compareOutputRoot(bytes32 stateRoot, bytes32 withdrawalStorageRoot, bytes32 latestBlockhash, uint256 blockNumber) public view returns(bool) {
        (,bytes32 outputRoot) = getBlockandRoot();
